...
def generate_keys_for_game(game, retries=3):
    keys = set()  # Use a set to avoid duplicates
    for _ in range(MAX_KEYS_PER_DAY):
        ...
        for attempt in range(game['attemptsNumber']):
            ...
            client_token = login(generate_client_id(), game['appToken'])  # Generate new client token before key generation
            if emulate_progress(client_token, game['promoId']):
                ...
                for _ in range(retries):
                    try:
                        ...
                        if key:
                            keys.add(key)  # Use set to avoid duplicates
                            break
                    except Exception as e:
                        print(f"Retry error for {game['name']}: {e}")
                if key:
                    print(f"Generated key for {game['name']}: {key}")
                    success = True
                    break
        if not success:
            print(f"Failed to generate key for {game['name']}")
    return list(keys)  # Convert set back to list for further use
...
def main():
    for day in range(KEYS_DAY_COUNT):
        ...
        for game_id, game in games.items():
            ...
            try:
                keys[game['name']] = generate_keys_for_game(game)
            except Exception as e:
                print(f"Error: {e}")
